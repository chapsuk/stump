...
var (
	...
	// Debug:
	//defaultLogger, _ = logger.New(&logger.Options{
...
func init() {
	...
	if conf, err = config.New(&config.Options{
		AutoEnv: true,
	}); err != nil {
		panic(err)
	}
	...
	database, err = db.New(db.Options{
		Addr:     conf.GetString("DATABASE_ADDR"),
		Username: conf.GetString("DATABASE_USER"),
		Password: conf.GetString("DATABASE_PASS"),
		Database: conf.GetString("DATABASE_DB"),
		PoolSize: 2,
	})
